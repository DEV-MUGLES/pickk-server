...
export class Init1633566593646 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'CREATE TABLE `seller_crawl_strategy` (`id` int NOT NULL AUTO_INCREMENT, `createdAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `updatedAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), `sellerId` int NOT NULL, `itemsSelector` varchar(100) NOT NULL, `codeRegex` varchar(30) NOT NULL, `pagination` tinyint NOT NULL, `pageParam` varchar(20) NULL, `baseUrl` varchar(150) NOT NULL, `startPathNamesJoin` varchar(255) NOT NULL, UNIQUE INDEX `REL_9eb9935015e8ba7813998c22f1` (`sellerId`), PRIMARY KEY (`id`)) ENGINE=InnoDB'
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      "CREATE TABLE `cart_item` (`id` int NOT NULL AUTO_INCREMENT, `createdAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `updatedAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), `productId` int NOT NULL, `userId` int NOT NULL, `recommendDigestId` int NULL, `quantity` smallint UNSIGNED NOT NULL DEFAULT '0', INDEX `idx-createdAt` (`createdAt`), PRIMARY KEY (`id`)) ENGINE=InnoDB"
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      "CREATE TABLE `shipment` (`id` int NOT NULL AUTO_INCREMENT, `createdAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `status` enum ('Shipping', 'Shipped', 'Cancelled', 'Failed') NOT NULL, `ownerType` enum ('ExchangeRequestPick', 'ExchangeRequestReship', 'RefundRequest', 'OrderItem') NULL, `ownerPk` varchar(30) NULL, `courierId` int NOT NULL, `trackCode` varchar(30) NOT NULL, `lastTrackedAt` datetime NULL, INDEX `idx-status` (`status`), PRIMARY KEY (`id`)) ENGINE=InnoDB"
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      "CREATE TABLE `order` (`merchantUid` char(20) NOT NULL, `createdAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `updatedAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), `userId` int NOT NULL, `status` enum ('Pending', 'Paying', 'Failed', 'VbankReady', 'VbankDodged', 'Paid') NOT NULL, `payMethod` enum ('card', 'trans', 'vbank', 'phone', 'samsungpay', 'kpay', 'kakaopay', 'payco', 'lpay', 'ssgpay', 'tosspay', 'cultureland', 'smartculture', 'happymoney', 'booknlife', 'point', 'naverpay', 'chaipay') NULL, `payingAt` datetime NULL, `failedAt` datetime NULL, `vbankReadyAt` datetime NULL, `vbankDodgedAt` datetime NULL, `paidAt` datetime NULL, `withdrawnAt` datetime NULL, `vbankReceiptId` int NULL, `buyerId` int NULL, `receiverId` int NULL, `refundAccountId` int NULL, UNIQUE INDEX `REL_86777bc5e52132792a38468161` (`vbankReceiptId`), UNIQUE INDEX `REL_20981b2b68bf03393c44dd1b9d` (`buyerId`), UNIQUE INDEX `REL_0ba887a07aa531f6c5821950ec` (`receiverId`), UNIQUE INDEX `REL_7b8ec169516a3cd948e47f2116` (`refundAccountId`), PRIMARY KEY (`merchantUid`)) ENGINE=InnoDB"
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'ALTER TABLE `seller` ADD CONSTRAINT `FK_af49645e98a3d39bd4f3591b334` FOREIGN KEY (`userId`) REFERENCES `user`(`id`) ON DELETE SET NULL ON UPDATE NO ACTION'
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'ALTER TABLE `seller_settle_account` ADD CONSTRAINT `FK_5f0b96ea075e9f5f98f5f61ca54` FOREIGN KEY (`settlePolicyId`) REFERENCES `seller_settle_policy`(`id`) ON DELETE CASCADE ON UPDATE NO ACTION'
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'ALTER TABLE `cart_item` ADD CONSTRAINT `FK_eb2a1ce690707db891ef7273e4e` FOREIGN KEY (`recommendDigestId`) REFERENCES `digest`(`id`) ON DELETE SET NULL ON UPDATE NO ACTION'
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'ALTER TABLE `order_item` ADD CONSTRAINT `FK_aa2ba1e9505e547dec18394a3b0` FOREIGN KEY (`recommendDigestId`) REFERENCES `digest`(`id`) ON DELETE SET NULL ON UPDATE NO ACTION'
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'ALTER TABLE `order_item` DROP FOREIGN KEY `FK_aa2ba1e9505e547dec18394a3b0`'
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'ALTER TABLE `cart_item` DROP FOREIGN KEY `FK_eb2a1ce690707db891ef7273e4e`'
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'ALTER TABLE `seller_settle_account` DROP FOREIGN KEY `FK_5f0b96ea075e9f5f98f5f61ca54`'
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'DROP INDEX `REL_5f0b96ea075e9f5f98f5f61ca5` ON `seller_settle_account`'
    );
