...
export class AddOrderOrderItem1626946767657 implements MigrationInterface {
  name = 'AddOrderOrderItem1626946767657';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      "CREATE TABLE `order_item` (`merchantUid` char(19) NOT NULL, `createdAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `updatedAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), `userId` int NULL, `sellerId` int NULL, `itemId` int NULL, `productId` int NULL, `orderMerchantUid` char NOT NULL, `status` enum ('pending', 'paying', 'failed', 'vbank_ready', 'paid', 'withdrawn', 'ship_pending', 'ship_ready', 'shipping', 'shipped') NOT NULL, `claimStatus` enum ('cancel_requested', 'cancelled', 'exchange_requested', 'exchanged', 'refund_requested', 'refunded') NULL, `quantity` smallint UNSIGNED NOT NULL, `isConfirmed` tinyint NOT NULL DEFAULT 0, `isSettled` tinyint NOT NULL DEFAULT 0, `itemFinalPrice` int UNSIGNED NOT NULL, `couponDiscountAmount` int UNSIGNED NOT NULL, `usedPointAmount` int UNSIGNED NOT NULL, `payAmount` int UNSIGNED NOT NULL, `brandNameKor` varchar(30) NOT NULL, `itemName` varchar(255) NOT NULL, `productVariantName` varchar(255) NOT NULL, `recommenderId` int NULL, `recommenderNickname` varchar(11) NULL, `recommendContentType` enum ('Pickk', 'Look') NULL, `recommendContentItemId` int NULL, `courierId` int NULL, `trackCode` varchar(30) NULL, `paidAt` datetime NULL, `withdrawnAt` datetime NULL, `shipReadyAt` datetime NULL, `shippingAt` datetime NULL, `shippedAt` datetime NULL, `cancelRequestedAt` datetime NULL, `cancelledAt` datetime NULL, `exchangeRequestedAt` datetime NULL, `exchangedAt` datetime NULL, `refundRequestedAt` datetime NULL, `refundedAt` datetime NULL, `shipReservedAt` datetime NULL, `confirmedAt` datetime NULL, `settledAt` datetime NULL, PRIMARY KEY (`merchantUid`)) ENGINE=InnoDB"
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      "CREATE TABLE `order` (`merchantUid` char(17) NOT NULL, `createdAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `updatedAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), `userId` int NULL, `status` enum ('pending', 'paying', 'failed', 'vbank_ready', 'paid', 'withdrawn') NOT NULL, `payMethod` enum ('pending', 'paying', 'failed', 'vbank_ready', 'paid', 'withdrawn') NULL, `totalItemFinalPrice` int UNSIGNED NOT NULL, `totalShippingFee` int UNSIGNED NOT NULL, `totalCouponDiscountAmount` int UNSIGNED NOT NULL, `totalUsedPointAmount` int UNSIGNED NOT NULL, `totalPayAmount` int UNSIGNED NOT NULL, `pendingAt` datetime NULL, `payingAt` datetime NULL, `failedAt` datetime NULL, `vbankReadyAt` datetime NULL, `paidAt` datetime NULL, `withdrawnAt` datetime NULL, `vbankInfoId` int NULL, `buyerId` int NULL, `receiverId` int NULL, INDEX `idx_merchantUid` (`merchantUid`), UNIQUE INDEX `REL_dd872794500469f141a2bc7438` (`vbankInfoId`), UNIQUE INDEX `REL_20981b2b68bf03393c44dd1b9d` (`buyerId`), UNIQUE INDEX `REL_0ba887a07aa531f6c5821950ec` (`receiverId`), PRIMARY KEY (`merchantUid`)) ENGINE=InnoDB"
    );
